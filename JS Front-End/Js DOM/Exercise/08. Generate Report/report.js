//thanks to mr papazov cuz i would never
...
function generateReport() {
    ...
    const columns = Array 
        .from(thElements)
        .map(thElement => {
            const checkedBoxElement = thElement.querySelector('input[type=checkbox]');

            return {
                name: thElement.textContent.toLowerCase().trim(),
                active: checkedBoxElement.checked
            }
        });
    ...
    outputElement.value = JSON.stringify(reportDetails, null, 2);
}